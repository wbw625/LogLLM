...
class LogLLM(nn.Module):
    def __init__(self, Bert_path, Llama_path, ft_path=None, is_train_mode=True, device = torch.device("cuda:0"), max_content_len = 128, max_seq_len = 128, max_new_tokens = 100, desc_loss_weight = 0.2):
        super().__init__()
        ...
        self.max_new_tokens = max_new_tokens  # max tokens to generate (label + description)
        self.desc_loss_weight = desc_loss_weight  # loss weight for description tokens (label tokens = 1.0)
        self.device = device
        ...
        post_prompt = '. Is this sequence normal or anomalous? Explain the reason if possible. \n'
    ...
    def train_helper(self, inputs, seq_positions, labels, descriptions):
        '''
        ...
        :param descriptions: np.array of description strings
        ...
        # 构建包含 label 和 description 的目标字符串
        # 有 description 的样本：输出 label + reason；没有的：只输出 label
        target_strings = []
        for label, desc in zip(labels, descriptions):
            if desc:
                target_strings.append(f"The sequence is {label}. {desc}")
            else:
                target_strings.append(f"The sequence is {label}.")
        target_strings = np.array(target_strings, dtype=object)
        ...
        # 构建 per-token loss 权重：label 部分权重=1.0，description 部分权重=desc_loss_weight，EOS 权重=1.0
        token_weights = torch.ones_like(target_tokens_atts, dtype=torch.float32)
        for i in range(batch_size):
            # 单独 tokenize label 部分，计算 label token 数量
            label_only = f"The sequence is {labels[i]}."
            label_token_len = len(self.Llama_tokenizer(label_only, add_special_tokens=False)['input_ids'])

            # 找到该样本中真实（非 padding）token 的位置
            real_positions = target_tokens_atts[i].nonzero(as_tuple=True)[0]
            # 如果只有 label + EOS，没有 description tokens，不需要调整
            if len(real_positions) <= label_token_len + 1:
                continue
            # description tokens 位于 label tokens 之后、EOS 之前
            desc_start = real_positions[label_token_len].item()
            eos_pos = real_positions[-1].item()
            token_weights[i, desc_start:eos_pos] = self.desc_loss_weight

        return Llama_output[label_mask], target_tokens_ids[target_tokens_atts], token_weights[target_tokens_atts]
    ...
    def forward(self, inputs, seq_positions):
        ...
        while not this_peer_finished:
            ...
            if self.max_new_tokens < len(answer):
                this_peer_finished = True
