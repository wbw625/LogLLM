...
from customDataset_description import CustomDataset, CustomCollator, BalancedSampler
...
max_seq_len = 128
max_new_tokens = 100
desc_loss_weight = 0.2  # description token 的 loss 权重 (label token = 1.0)

data_path = r'/data/fangly/shqxBS/log/data/{}/train01_desc.csv'.format(dataset_name)
...
f'desc_loss_weight: {desc_loss_weight}\n'
...
def trainModel(model, dataloader, gradient_accumulation_steps, n_epochs, lr):
    ...
    for epoch in range(int(n_epochs)):
        ...
        for i_th, bathc_i in enumerate(pbar):
            ...
            descriptions = bathc_i['descriptions']

            inputs = inputs.to(device)
            seq_positions = seq_positions

            outputs, targets, weights = model.train_helper(inputs, seq_positions, labels, descriptions)

            loss_per_token = criterion(outputs, targets)
...
if __name__ == '__main__':
    ...
    model = LogLLM(Bert_path, Qwen_path, device = device, max_content_len = max_content_len, max_seq_len = max_seq_len, max_new_tokens = max_new_tokens, desc_loss_weight = desc_loss_weight)
